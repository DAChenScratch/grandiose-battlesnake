from utils import assign, getBodies, getHeads, getPossibleMoves, safe, onlyOneWayToGo, indvBodies, areas_near_heads, smaller_snakes_get_heads, get_dangerous_heads
...
def best_move(data, scores):
  ...
  for a in range(4):
    ...
    if safe(dict_moves[a], heads, indv_bodies, myhead, mybody) == []:
      ...
      move_count = getPossibleMoves(bodies_copy, dict_moves[a], data, body_copy)
      points_moves[a] += (move_count - len(mybody)/scores[1]) #1.5
      print("MOVES AVAILABLE FOR MOVE", moves[a], ': ' + str(move_count))
      
      points_moves[a] -= scores[2]
    else:
      points_moves[a] -= scores[2] * 2.5 #70
    print('MOVESCORE FOR', moves[a], 'SO FAR:', points_moves[a])
    if me_smallest == 2:
      print('I AM THE SMALLEST SNAKE')
    if health < 46 or me_smallest == 2:
      for foodcoord in food:
        points_moves[a] -=distance_between(dict_moves[a], foodcoord) * 2
    ...
    if me_smallest !=2:
      for smallhead in heads:
        avg = (width + height) / 2
        points_moves[a] += (2 * avg) - (distance_between(dict_moves[a], smallhead))
